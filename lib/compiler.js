...
/// @summary Defines the various types of IPC messages between the CompilerCache
///  and a data compiler process. This enumeration must be kept in sync with the
...
/// @summary Constructor function for the JobQueue type, which provides a simple
/// method for queueing and executing some sort of user-defined task, where the
/// thing that does the work can only process a single request at a time.
...
/// @summary Checks the queue status to determine whether the queue is empty.
...
/// @summary Retrieves the item at the front of the queue.
...
/// @summary Enqueues a job. If the queue is currently empty, the job is
...
/// @summary This method should be called when the most recently executed job
/// has completed, whether the job completed successfully or with an error. The
...
/// @summary Flushes all pending jobs from the queue.
JobQueue.prototype.flush = function ()
{
    this.jobList = [];
};

/// @summary Constructor function for the CompilerCache type, which spawns data
/// compiler processes based on a pipeline definition file and handles the
/// dispatching of data compilation requests to them.
...
/// @summary Evaluates a condition to determine whether all data compiler
/// processes have been spawned successfully.
...
/// @summary Emits the 'ready' event to indicate that this CompilerCache
...
/// @summary Sends an IPC message to a data compiler process requesting the
/// version information for the data compiler.
...
/// @summary Callback invoked when a data compiler is ready to begin another
/// compile operation. When the data compiler completes, either successfully or
/// with an error, it sends an IPC message to its Monitor.
...
/// @summary Callback invoked when a data compiler has completed all tasks.
...
/// @summary Event handler invoked when a monitored child process has started
...
/// @summary Event handler invoked when a monitored child process cannot be
/// started. This event can occur on either the initial start, or a subsequent
/// restart of the child process.
...
/// @summary Event handler invoked when a monitored child process is restarted.
/// @param monitor The Monitor instance that raised the event.
...
/// @summary Event handler invoked when a monitored child process sends the the
/// parent process (this process) an IPC message.
/// @param monitor The Monitor instance that raised the event.
...
/// @summary Event handler invoked when a monitored child process has terminated
/// fully and will not be restarted. This is typically due to a call to the
/// function Monitor.kill(false) to shut down the system.
/// @param monitor The monitor instance associated with the child process.
...
/// @summary Constructs a string representing a monitor configuration instance.
/// This string may be used as a key to uniquely identify a process
/// configuration and prevent spawning of duplicate processes when the same
/// compiler is used by more than one resource type.
...
/// @summary Launches and begins monitoring of processes for all data compilers
/// in a content pipeline configuration object.
...
/// @summary Terminates all data compiler processes. When all processes have
/// terminated, the CompilerCache emits a 'terminated' event.
...
/// @summary Locates the compiler instance for a particular combination of
/// resource type and target platform.
...
/// @summary Submits a source file to be built.
...
/// @summary Creates a new CompilerCache optionally initialized with a content
/// pipeline definition. If a pipeline definition is supplied, all data compiler
