...
/// @summary Loads a content pipeline definition from the filesystem.
...
/// @summary Saves a content pipeline definition to the filesystem.
...
/// @summary Loads a list of recognized platform identifiers.
...
/// @summary Saves a platform identifier list to the filesystem.
...
/// @summary Constructor function for the Target type, which represents the
/// output location for content files for a particular target platform.
/// Instances of this type are typically created using Target.create().
...
/// @summary The name of the generic platform is just an empty string.
Target.GENERIC_PLATFORM    = 'generic';

/// @summary The directory extension used for target packages.
Target.TARGET_EXTENSION    = '.target';

/// @summary The file extension used for target databases.
Target.TARGET_DB_EXTENSION = '.target.json';

/// @summary The file extension used for source databases.
Target.SOURCE_DB_EXTENSION = '.source.json';

/// @summary Loads and caches the data representing the output location and
/// metadata for content files built for a specific target platform.
...
/// @summary Constructs the target path associated with a given resource name.
...
/// @summary Constructor function for the Package type, which represents a
/// logical group of content. Instances of this type are typically created
/// using the @a Package.create() function.
...
/// @summary The directory extension used for source packages.
Package.SOURCE_EXTENSION    = '.source';

/// @summary Loads and caches the data representing the source location, output
...
/// @summary Retrieves the data associated with a particular target platform
/// for this content package. The target platform record is created if it
/// doesn't exist.
...
/// @summary Scans the filesystem and creates Target platform records for any
/// target directories it finds belonging to the content package.
...
/// @summary Constructor function for the Project type, which stores paths
/// associated a content project on the filesystem. Instances of this type are
/// typically created using the @a Project.create() function.
...
/// @summary A string specifying the name of the root directory for content
/// processors executables.
...
/// @summary A string specifying the name of the root directory for source and
/// target database files.
Project.DATABASE_DIRECTORY   = 'database';

/// @summary A string specifying the name of the pipeline configuration file.
Project.PIPELINE_FILE        = 'pipeline.json';

/// @summary The name of the file containing the list of recognized target
/// platforms.
Project.PLATFORM_FILE        = 'platform.json';

/// @summary Loads and caches the data associated with a content project, which
/// acts as a container for content packages.
/// @param args An object specifying information about the environment.
/// @param args.projectName The name of the parent project.
...
/// @summary Retrieves the data associated with a particular content package
/// for this project. The content package record is created if it doesn't exist.
...
/// @summary Scans the filesystem and creates Package records for any source
/// content directories it finds belonging to the content project.
...
/// @summary Constructor function for a type that implements the build process
/// for a single content package and target platform combination.
...
/// @summary Performs a quick check to determine if a source file has been
/// modified by checking the modification time and file size.
...
/// @summary Determines whether any of the dependencies of a given source file
/// have been modified by examining modification time and file size.
...
/// @summary Determines whether the build outputs for a given target resource
/// exist on disk or not.
...
/// @summary Checks a given resource to determine whether it needs to be built.
...
/// @summary Callback invoked when the CompilerCache emits a 'skipped' event to
/// signal that it skipped building a source file, typically because no data
/// compiler exists for the file's resource type.
...
/// @summary Callback invoked when the CompilerCache emits a 'started' event to
/// signal that a file build has been submitted to a data compiler.
/// @param compilers The CompilerCache instance that raised the event.
...
/// @summary Callback invoked when the CompilerCache emits a 'complete' event
/// to signal that the data compiler has finished executing the build for a
/// given source content item.
/// @param compilers The CompilerCache instance that raised the event.
/// @param result An object describing the result of the build operation.
/// @param result.input An object describing the input parameters.
/// @param result.input.bundle The Package instance for the content bundle.
...
/// @summary Given a set of properties associated with a resource, determine
/// the one identifying the target platform.
...
/// @summary Reports that a source file is being skipped for some reason.
...
/// @summary Examines the data for a filesystem entry to determine whether it
/// should be considered as a source file for the current build target.
...
/// @summary Determines the set of source files considered part of the build
/// target.
...
/// @summary Examines the set of build target source files to determine which
/// files actually need to be rebuilt. Events indicating that files are
/// up-to-date are emitted during this call.
...
/// @summary Submits a set of files to their corresponding data compilers to be
/// processed.
...
/// @summary Determines whether the package manifest file exists on the
/// filesystem.
...
/// @summary Creates a package manifest, which contains the metadata for all of
/// the package resources, and writes it to the target directory.
/// @param manifestName The name and extension of the package manifest file.
...
/// @summary The name of the manifest file for a content package.
PackageBuilder.MANIFEST_NAME = 'package.manifest';

/// @summary Handles the 'started' event emitted by a TargetBuilder instance.
...
/// @summary Handles the 'skipped' event emitted by a TargetBuilder instance.
/// @param sender The TargetBuilder that raised the event.
/// @param fileInfo An object with additional information related to the event.
/// @param fileInfo.projectName The name of the project being built.
/// @param fileInfo.packageName The name of the content package being built.
...
/// @summary Handles the 'success' event emitted by a TargetBuilder instance.
/// @param sender The TargetBuilder that raised the event.
/// @param fileInfo An object with additional information related to the event.
/// @param fileInfo.projectName The name of the project being built.
/// @param fileInfo.packageName The name of the content package being built.
...
/// @summary Handles the 'error' event emitted by a TargetBuilder instance.
/// @param sender The TargetBuilder that raised the event.
/// @param fileInfo An object with additional information related to the event.
/// @param fileInfo.projectName The name of the project being built.
/// @param fileInfo.packageName The name of the content package being built.
...
/// @summary Checks the number of completion events still to be expected in
/// order to determine whether the content package has finished building.
...
/// @summary Notifies any event listeners that the content package build has
/// finished.
...
/// @summary Notifies any event listeners that the content package build is starting.
/// @param targetBuilder The TargetBuilder instance representing the build.
...
/// @summary An internal function called when the build for a particular target
/// has finished. This method saves any database files, generates the package
...
/// @summary Starts building the content package for a particular target
/// platform.
...
/// @summary Inspects the filesystem to determine the known target platforms.
...
/// @summary Constructor function for a type that loads projects from disk,
/// starts the content pipeline processes they have defined, and can enumerate
/// and build content packages for the project.
...
/// @summary Callback invoked when an error occurs when spawning a data
...
/// @summary Callback invoked when all compiler processes for the content
/// pipeline have been spawned and are ready to begin accepting work.
/// @param compilers The CompilerCache that raised the event.
...
/// @summary Callback invoked when all compiler processes for the content
/// pipeline have been fully terminated.
/// @param compilers The CompilerCache that raised the event.
...
/// @summary Loads (or creates) a project from disk, and starts the processes
/// defined for its content pipeline. When the project and its content pipeline
/// are ready to be built, a 'ready' event is emitted.
...
/// @summary Examines the filesystem to determine what content packages are
/// part of the project, and returns the package names.
...
/// @summary Creates an object that can be used to build a specific content
/// package.
/// @param packageName The name of the content package.
...
/// @summary Stops all processes that are part of the content pipeline for the
/// current project. Once all processes have stopped, a 'disposed' event is
/// emitted.
/// @return A reference to the ProjectBuilder.
...
/// @summary Loads (or creates) a project from disk.
/// @param projectPath The path of the project directory for the project to
/// load or create. This will be resolved to an absolute path.
...
/// @summary Creates or loads a content project from disk.
...
/// @summary Creates an object that can manage the build process for a project.
