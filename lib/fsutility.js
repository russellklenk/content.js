...
/// @summary Implements proper value defaulting for types which may have falsey
/// values, like integers and booleans.
...
/// @summary Constructs a new FSEntry instance, which represents a single file
/// or directory in a filesystem tree.
...
/// @return A reference to the new FSEntry instance.
var FSEntry = function (rootPath, entryName, absPath, statInfo)
{
    if (!(this instanceof FSEntry))
        return new FSEntry(rootPath, entryName, absPath, statInfo);
...
/// @summary Serializes a tree of FSEntry instances to a JSON-encoded string.
...
/// @summary Reconstructs a tree of FSEntry instances from a JSON-encoded form.
...
/// @summary Queries the entry to determine whether it references a file.
...
/// @summary Queries the entry to determine whether it references a directory.
...
/// @summary Searches for a named file located within the directory.
...
/// @summary Searches for a named subdirectory located within the directory.
...
/// @summary The object representing the FSScanner type, which can scan a
/// directory tree and return information as an FSEntry. All methods are static.
var FSScanner = {};

/// @summary Builds a tree of FSEntry instances describing the current state of
/// the filesystem from a particular root path.
...
/// @summary Scans a directory on the filesystem, returning information about
/// the directory contents as an FSEntry instance.
/// @param options An object controlling the scanner behavior.
/// @param options.from The root path at which to begin the scan. This path is
/// resolved into an absolute path prior to use. Defaults to process.cwd().
/// @param options.recursive Specify true to recurse into subdirectories. The
...
/// @summary The object representing the FSDiffer type, which can determine the
/// differences between two in-memory copies of a filesystem tree. All methods
/// are static.
var FSDiffer = {};

/// @summary Creates an empty difference list object for entries rooted at a
/// given path.
...
/// @summary Determines the differences in the files and subdirectories of two
/// versions of a given filesystem tree.
/// @param root The absolute root path for all FSEntry data.
...
/// @summary Determines the differences between the files in two directories.
/// @param a The FSEntry instance representing the 'previous' or 'left-hand'
/// version of the directory.
/// @param b The FSEntry instance representing the 'current' or 'right-hand'
/// version of the directory.
...
/// @summary Writes path information for a difference list to the console.
...
/// @summary Constructor function for a type that can monitor a directory tree
/// and report file and directory additions, deletions and modifications. The
...
/// @return A reference to the new FSWatcher instance.
...
/// @summary Scans the filesystem starting from the root path.
...
/// @summary Diffs the filesystem starting from the root path.
...
/// @summary Handles a single update cycle for the watcher, swapping the
/// filesystem trees, rescanning, diffing and possibly emitting events.
...
/// @summary Starts monitoring the current root directory.
...
/// @summary Stops monitoring the current root directory.
/// @return A reference to the FSWatcher instance.
...
/// @summary Ensure that a value ends in the path separator character.
...
/// @summary Ensure that a value does not end in a path separator character.
...
/// @summary Synchronously walks a filesystem tree, executing a user-defined
/// callback for each file or directory.
...
/// @summary Synchronously walks a filesystem tree, executing a user-defined
/// callback for each file or directory. The root directory is NOT included.
...
/// @summary Synchronously creates a directory tree. Any directories in the path
/// that don't exist are created. Shamelessly ripped from node-mkdirp.
...
/// @summary Synchronously reads the contents of a directory and all child
/// directories. Shamelessly ripped from wrench.js.
...
/// @summary Synchronously determines whether a given path exists and represents
/// a file by querying the filesystem.
...
/// @summary Synchronously determines whether a given path exists and represents
/// a directory by querying the filesystem.
/// @param path The path to check.
...
/// @summary Writes an numeric value to a tar header block. The value is written
/// as a string of ASCII-encoded octal digits. Space permitting, an ASCII space
...
/// @summary Writes a string value to a tar header block. The value is written
/// as a series of ASCII-encoded characters. Any remaining space is filled with
/// zero bytes until the desired length is achieved.
/// @param buffer The destination buffer.
/// @param offset The byte offset within @a buffer at which to begin writing.
/// @param length The number of bytes to write.
...
/// @summary Writes the tar header entry for a file to a buffer. This function
/// only handles ASCII character paths properly; paths with unicode characters
/// may not be written to the archive without data loss.
...
/// @summary Constructs a tar archive file from the filesystem and writes it
/// to disk.
