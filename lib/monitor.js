...
/// obnoxious code every time. The typical javascript '||' trick works for
...
/// @summary Constructor function for a Monitor object, which executes a script
/// as a child process and restarts it if it fails.
...
/// @return A reference to the new Monitor instance.
...
/// @summary Builds an object whose key-value pairs represent a given
/// environment configuration. The caller can hide environment information
/// exposed by the process environment, as well as add new key-value pairs.
...
/// @summary Event handler invoked when an IPC message is received from the
/// child process. The message object is re-emitted as a 'message' event on the
/// Monitor instance.
...
/// @summary Event handler invoked when the child process has exited. The
/// monitor will either let the child process die completely, or it will attempt
/// to restart the child process, depending on whether Monitor.kill() was called
/// with the allowRestart value set to false or true. If the child process exits
/// for some reason (it crashed) it is restarted.
...
/// @summary Event handler invoked when the IPC channel to the child process
...
/// @summary Starts (or restarts) the child process. If the child process is
/// already running, the function returns immediately and no events are raised.
/// Emits either the 'start' or 'restart' event, each of which specify the
/// Monitor instance that started the child process. An 'error' event is emitted
/// if the child process cannot be started. All events are emitted on the next
...
/// @summary Signals the child process to exit, allowing the caller to specify
/// whether or not to allow the process to automatically restart. Emits the
/// 'stop' event on the current event loop tick. The 'exit' event is not emitted
/// until the child process has fully terminated.
...
/// @summary Sends a message to the child process via the built-in IPC
/// mechanism. See the docs for ChildProcess.send(message, [sendHandle]).
...
/// @summary Create a new Monitor instance initialized with the specified
/// configuration data.
